...
<div class="max-w-4xl" x-data="{ loading: false }">
    ...
    <div class="card">
        <div class="card-header">
            <h2 class="text-lg font-semibold text-gray-900">Edit Pelanggan: {{ $customer->name }}</h2>
            <p class="mt-1 text-sm text-gray-500">Perbarui data pelanggan yang sudah ada.</p>
        </div>

        <form method="POST" action="{{ route('admin.customers.update', $customer) }}" @submit="loading = true">
            @csrf
            @method('PUT')
            ...
            <div class="card-footer flex items-center justify-end gap-3">
                ...
                <button type="submit" class="btn btn-primary" :disabled="loading">
                    ...
                    <span x-text="loading ? 'Memperbarui...' : 'Perbarui Pelanggan'">Perbarui Pelanggan</span>
                </button>
            </div>
        </form>
    </div>
