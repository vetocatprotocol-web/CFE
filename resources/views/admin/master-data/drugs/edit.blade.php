...
<div class="max-w-4xl" x-data="{ loading: false }">
    ...
    <div class="card">
        <div class="card-header">
            <h2 class="text-lg font-semibold text-gray-900">Edit Obat: {{ $drug->name }}</h2>
            <p class="mt-1 text-sm text-gray-500">Perbarui data obat yang sudah ada.</p>
        </div>

        <form method="POST" action="{{ route('admin.drugs.update', $drug) }}" @submit="loading = true">
            @csrf
            @method('PUT')
                        ...
                        <div class="form-group">
                            ...
                            <select name="unit" id="unit" required
                                ...
                                @foreach(['tablet', 'kapsula', 'botol', 'vial', 'ampul', 'gram', 'ml', 'tube'] as $unit)
                                    <option value="{{ $unit }}" {{ old('unit', $drug->unit) === $unit ? 'selected' : '' }}>{{ ucfirst($unit) }}</option>
                                @endforeach
                            </select>
                            @error('unit')
                                <p class="form-error">{{ $message }}</p>
                ...
                <div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="description" id="description" rows="3"
                                  ...
                                  placeholder="Deskripsi obat...">{{ old('description', $drug->description) }}</textarea>
                        @error('description')
                            <p class="form-error">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div class="form-group">
                            ...
                            <input type="number" name="price_per_unit" id="price_per_unit" value="{{ old('price_per_unit', $drug->price_per_unit) }}" required min="0" step="500"
            ...
            <div class="card-footer flex items-center justify-end gap-3">
                ...
                <button type="submit" class="btn btn-primary" :disabled="loading">
                    ...
                    <span x-text="loading ? 'Memperbarui...' : 'Perbarui Obat'">Perbarui Obat</span>
                </button>
            </div>
        </form>
    </div>
