...
@section('header-title', 'Master Data - Layanan')
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($services as $service)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <span class="px-2.5 py-0.5 text-xs font-medium rounded-full {{ $service->status === 'ACTIVE' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700' }}">
                                    {{ $service->status }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('admin.services.edit', $service) }}" class="text-primary-600 hover:text-primary-800 p-1" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </a>
                                    <form method="POST" action="{{ route('admin.services.destroy', $service) }}" x-data="{ show: false }">
                                        @csrf
                                        @method('DELETE')
                                        ...
                                        <div x-show="show" x-cloak x-transition @click.away="show = false" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
                                            <div class="bg-white rounded-xl shadow-xl max-w-md w-full mx-4 p-6" @click.stop>
                                                ...
                                                <p class="text-sm text-gray-600 mb-6">Apakah Anda yakin ingin mengarsipkan layanan <strong>{{ $service->name }}</strong>? Layanan tidak akan muncul di daftar aktif.</p>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-8 text-center text-sm text-gray-500">Tidak ada layanan ditemukan</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
