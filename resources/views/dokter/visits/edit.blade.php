...
@section('title', 'Edit Kunjungan - ' . $visit->visit_number)
@section('header-title', 'Edit Kunjungan')

@section('content')
<div x-data="{
    searchQuery: '',
    searchResults: [],
    selectedCustomer: @json($visit->customer ? ['id' => $visit->customer->id, 'name' => $visit->customer->name, 'phone' => $visit->customer->phone, 'pets' => $visit->customer->pets->map(fn($p) => ['id' => $p->id, 'name' => $p->name, 'species' => $p->species, 'breed' => $p->breed])] : null),
    ...
    drugs: @json($drugs),
    items: @json($visit->items->map(fn($item) => [
        'item_type' => $item->item_type,
        'service_id' => $item->service_id,
        'drug_id' => $item->drug_id,
        'item_name' => $item->service->name ?? $item->drug->name ?? '',
    ...
    <form method="POST" action="{{ route('dokter.visits.update', $visit) }}" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <textarea name="chief_complaint" rows="3" required
                              ...
                              placeholder="Keluhan utama pasien...">{{ old('chief_complaint', $visit->chief_complaint) }}</textarea>
            ...
            <div class="mt-4">
                ...
                <textarea name="diagnosis" rows="2"
                          ...
                          placeholder="Diagnosis...">{{ old('diagnosis', $visit->diagnosis) }}</textarea>
                @error('diagnosis')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mt-4">
                ...
                <textarea name="treatment_notes" rows="2"
                          ...
                          placeholder="Catatan perawatan...">{{ old('treatment_notes', $visit->treatment_notes) }}</textarea>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4">
                <div>
                    ...
                    <input type="number" name="weight_kg" step="0.1" min="0" value="{{ old('weight_kg', $visit->weight_kg) }}"
                ...
                <div>
                    ...
                    <input type="number" name="temperature" step="0.1" min="0" value="{{ old('temperature', $visit->temperature) }}"
                ...
                <div>
                    ...
                    <input type="number" name="heart_rate" min="0" value="{{ old('heart_rate', $visit->heart_rate) }}"
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div x-show="items.length === 0" class="text-center py-8 text-sm text-gray-500">
                Belum ada layanan atau obat ditambahkan.
            </div>
        ...
        <div class="flex items-center justify-end gap-3">
            <a href="{{ route('dokter.visits.show', $visit) }}" class="px-4 py-2.5 text-sm font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors">
                Batal
            </a>
            <button type="submit"
